...
namespace ConsoleInput
{
    public static class Keyboard
    {
        ...
        private static Dictionary<VK, Keystate> StateCurrent = new Dictionary<VK, Keystate>();
        private static Dictionary<VK, Keystate> StatePrevious = new Dictionary<VK, Keystate>();
        
        private static Dictionary<VK, bool> keyboardKeyDown = new Dictionary<VK, bool>();
        //private static Dictionary<VK, ushort> keyboardRepeatCount = new Dictionary<VK, ushort>();

        public static bool KeyDown(VK key)
        {
        ...
        public static bool KeyPress(VK key)
        {
            if (!StateCurrent.TryGetValue(key, out Keystate state))
                return false;
        ...
        {
            StateCurrent = new Dictionary<VK, Keystate>();

            foreach (KeyValuePair<VK, bool> item in keyboardKeyDown)
            {
                ...
                else if (item.Value) // Key was not held down before but is now
                {
                    StateCurrent[item.Key] = Keystate.Press;
                }
            }

            keyboardKeyDown = new Dictionary<VK, bool>();
            StatePrevious = new Dictionary<VK, Keystate>(StateCurrent);
        }
